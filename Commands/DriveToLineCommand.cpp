...
DriveToLineCommand::DriveToLineCommand(float distanceToDrive) {
...
void DriveToLineCommand::Execute() {
	...
	driveSpeed = ((driveDistance - driveProgress)/driveDistance) * decayFactor * maxSpeed;
	if (driveSpeed > 1.0) driveSpeed  = maxSpeed; // just in case, make sure it doesn't drive too fast
