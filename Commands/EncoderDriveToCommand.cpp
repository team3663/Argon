...
EncoderDriveToCommand::EncoderDriveToCommand(float distanceToDrive, float maxSpeed, float decayFactor) {
...
void EncoderDriveToCommand::Execute() {
	...
	// set drive speed
	driveSpeed = ((distanceToDrive - driveProgress)/distanceToDrive) * decayFactor * maxSpeed;
...
bool EncoderDriveToCommand::IsFinished() {
	if (driveProgress >= distanceToDrive) // if we are at or past the target distance
		return true;
