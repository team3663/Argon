...
#include "AutonomousNinetyCommand.h"
AutonomousNinetyCommand::AutonomousNinetyCommand() {
	// Use requires() here to declare subsystem dependencies
	// eg. requires(chassis);
	// BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=REQUIRES
    // END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=REQUIRES
	AddSequential(new SetShootSpeedCommand(0.85));
	AddSequential(new IncreasePitchCommand(1.0, 6.0)); // set riser to top while spinning up
...
void AutonomousNinetyCommand::Initialize() {
	/*
...
void AutonomousNinetyCommand::Execute() {
	
}
// Make this return true when this Command no longer needs to run execute()
bool AutonomousNinetyCommand::IsFinished() {
	return false;
}
// Called once after isFinished returns true
void AutonomousNinetyCommand::End() {
	
}
// Called when another command which requires one or more of the same
// subsystems is scheduled to run
void AutonomousNinetyCommand::Interrupted() {
}
